...
metadata = {
    'protocolName': 'Protocol#2: Solvent dilution and supernatant transfer to Two 384-Well Plates for LC Analysis.',
    'author': 'Maria Cardelino + Catherine Mullins',
    'description': 'Adds 60 uL solvent (water for C18, 1:1 ACN/water for HILIC) to each well of respective 384-well plates. Then adds 30 uL supernatant to each well of both 384-well plates (C18 and HILIC) for LC analysis.'
}
...
def run(protocol: protocol_api.ProtocolContext):
    ...
    solvent_asp_rate_ACNWATER = 60 # ul/sec 
    solvent_disp_rate_ACNWATER = 40 # ul/sec
    ...
    sup_disp_rate = 30 #ul/sec 
    
    #### Equipment Locations ######################################
    
    ## Location for tips

    #ONLY need 10 columns of pipette  tips per rack.    
    tiprack_200_hilic = protocol.load_labware(
        'opentrons_flex_96_tiprack_200ul', 'B3', 
        adapter='opentrons_flex_96_tiprack_adapter')  # 200 uL tips for HILIC
...
#6 columns!
    ...
    # HILIC - water/ACN
    reservoir_acn_water = protocol.load_labware('nest_1_reservoir_195ml', 'B2') # HILIC reservoir 
    ...
    prewet(pipette, solvent_blow_rate, solvent_asp_rate_ACNWATER, solvent_disp_rate_ACNWATER, solvent_vol, reservoir_acn_water, Well, solvent_airgap_rate, airgap, protocol)

    solvent_addition(pipette, solvent_blow_rate, solvent_asp_rate_ACNWATER, solvent_disp_rate_ACNWATER, solvent_vol, reservoir_acn_water, hilic_plate, Well, solvent_airgap_rate, airgap, "A1")
    
    pipette.move_to(reservoir_acn_water["A1"].top(20), force_direct = True)     
    solvent_addition(pipette, solvent_blow_rate, solvent_asp_rate_ACNWATER, solvent_disp_rate_ACNWATER, solvent_vol, reservoir_acn_water, hilic_plate, Well, solvent_airgap_rate, airgap, "A2")
    
    pipette.move_to(reservoir_acn_water["A1"].top(20), force_direct = True)     
    solvent_addition(pipette, solvent_blow_rate, solvent_asp_rate_ACNWATER, solvent_disp_rate_ACNWATER, solvent_vol, reservoir_acn_water, hilic_plate, Well, solvent_airgap_rate, airgap, "B1")
    ...
    pipette.configure_nozzle_layout(
        style=ALL,
        start="A1", 
        tip_racks=[tiprack_50_1, tiprack_50_2, tiprack_50_3] # 50 uL tips at D3 ; will be changed 2 times for a total of 3 boxes.
    )
    ...
    transfer_supernatant(pipette, source_plate, c18_plate, hilic_plate, tiprack_50_1, sup_asp_rate, sup_disp_rate, sample_vol, airgap_sample, blow_height, asp_height, "A1", protocol)

    transfer_supernatant(pipette, source_plate, c18_plate, hilic_plate, tiprack_50_2, sup_asp_rate, sup_disp_rate, sample_vol, airgap_sample, blow_height, asp_height, "A2", protocol)

    transfer_supernatant(pipette, source_plate, c18_plate, hilic_plate, tiprack_50_3, sup_asp_rate, sup_disp_rate, sample_vol, airgap_sample, blow_height, asp_height, "B1", protocol)
...
def transfer_supernatant(pte, source_plate, c18_plate, hilic_plate, tiprack, sup_asp, sup_disp, svol, air, blow, x, well_id, ctx):
    ...
    pte.flow_rate.aspirate = sup_asp  
    pte.aspirate(svol, source_plate[well_id].bottom(x)) # x = asp_height (3.75 mm) above well bottom.
    ...
    pte.dispense(svol + air, c18_plate[well_id].bottom(blow)) 
   ...
   #### HILIC PLATE #####################################
    ...
    pte.flow_rate.dispense = sup_disp 
    pte.dispense(svol + air, hilic_plate[well_id].bottom(blow))  # Dispense the supernatant and air 12 mm from BOTTOM
